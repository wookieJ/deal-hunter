...
| Current focus | v0.7: sortable report (match / price / distance); one report template |
...
| 2026-08-25 | Report sorts client-side; unknown values sink and are dimmed | Sorting must work from a file:// URL with no server; treating a missing price as 0 would put it first | Cards carry data-score/price/km; behaviour verified by running the shipped script under a DOM shim |
| 2026-08-25 | Dropped the per-profile report template | It duplicated the tabbed report's markup and would have diverged (sorting in one, not the other) | `reports/index.html` is the only report |
...
| `./test.sh` | 84 unit tests | Extraction, scoring, rules, geometry, value, dedup, engine purity |
