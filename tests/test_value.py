...
SPEC = domains.load('bikes')['value_model']


class TestValueModel(unittest.TestCase):
    def test_better_groupset_estimates_higher(self):
        low = value.estimate({"groupset_tier": 42, "frame_material": "aluminium"}, SPEC)
        high = value.estimate({"groupset_tier": 92, "frame_material": "aluminium"}, SPEC)
        self.assertGreater(high, low)

    def test_carbon_is_worth_more_than_aluminium(self):
        alu = value.estimate({"groupset_tier": 72, "frame_material": "aluminium"}, SPEC)
        carbon = value.estimate({"groupset_tier": 72, "frame_material": "carbon"}, SPEC)
        self.assertGreater(carbon, alu)

    def test_older_bikes_are_worth_less(self):
        new = value.estimate({"groupset_tier": 72, "model_year": 2025}, SPEC)
        old = value.estimate({"groupset_tier": 72, "model_year": 2012}, SPEC)
        self.assertGreater(new, old)

    def test_clear_bargain_earns_points(self):
        attrs = {"groupset_tier": 90, "frame_material": "carbon", "brakes": "hydraulic_disc"}
        points, note = value.bargain(3000, attrs, SPEC)
        self.assertGreater(points, 0)
        self.assertIn("bargain", note)

    def test_overpriced_for_spec_is_penalised(self):
        points, note = value.bargain(5000, {"groupset_tier": 28, "frame_material": "aluminium"}, SPEC)
        self.assertLess(points, 0)
        self.assertIn("expensive", note)

    def test_fairly_priced_bike_is_neutral(self):
        points, _ = value.bargain(3200, {"groupset_tier": 60, "frame_material": "aluminium"}, SPEC)
